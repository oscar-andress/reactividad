...
- **`RouterConfig`** wires HTTP routes for `/api/v1/menu` (`GET /{menuId}`, `POST /`, `PUT /{menuId}`, `DELETE /{menuId}`) and registers `onError` mappings from domain exceptions to `GlobalExceptionHandler` methods. Any new domain exception needs both a handler method in `GlobalExceptionHandler` and an `onError(...)` registration here — WebFlux won't dispatch it otherwise.
